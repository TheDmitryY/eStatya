...
class AdminService:
    ...
    async def get_all_banned_users(self, skip: int, limit: int) -> List[ResponseUserDTO]:
        banned_users = await self.admin_repo.get_banned_users(skip=skip, limit=limit)
        if not users:
    ...
    async def ban_user_by_email(self, email: str) -> ResponseUserDTO:
        user = await self.admin_repo.ban_user_by_email(email=email)
        if not users:
            raise BanException("Users not found")
        return user